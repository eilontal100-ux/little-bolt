...
module.exports = function harness() {
 ...
 const source = gameClient.toString().replace("window.addEventListener('resize', resize);", `window.test = { update, frame, loseLife, restart, get enemies(){return enemies}, get lives(){return lives}, get damageTimer(){return damageTimer}, get state(){return state}, solids, WORLD, get coins(){return coins}, get coinCount(){return coinCount}, keys, touches, clearInput, get player(){return player}, get camera(){return camera}, set camera(v){camera=v}, get viewWidth(){return viewWidth}, set viewWidth(v){viewWidth=v}, get pickups(){return pickups}, get shards(){return shards}, get shield(){return shield}, get leapTimer(){return leapTimer}, get checkpoint(){return checkpoint}, jump(){keys.add("jump");jumpQueued=true}, releaseJump(){keys.delete("jump");releaseJump()} }; window.addEventListener('resize', resize);`);
